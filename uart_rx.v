module uart_rx(
    ...
    output reg        parity_result  ,  // 0是校验通过
    output reg        done           ,
    output reg        error          
);
...
always @(*) begin
    case (state)
        ...
        DATA_BITS: begin
            if ((parity_mode_reg == NO_PARITY) && (bit_cnt == 4'd7))
                next_state = STOP_BIT;
...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if (sys_rst_n == 0) begin
        ...
        case (state)
            ...
            PARITY: begin
                if (bit_flag == 1'b1) begin
                    if ((parity_mode_reg == ODD_PARITY) && (rx_reg3 == ~^rx_reg))
                        parity_result <= 0;
                    else if ((parity_mode_reg == EVEN_PARITY) && (rx_reg3 == ^rx_reg))
                        parity_result <= 0;
                    else
                        parity_result <= 1'b1;
                end
            ...
            STOP_BIT: begin
                ...
                end else if ((bit_flag == 1'b1) && (rx_reg3 == 0)) begin
                    error <= 1'b1;
                end
            end
        endcase
